...
// Componente visualizzazione card componente : Pannello Solare
// Props:
//          power : potenza pannello
//          amp :   amperaggio pannello
//          serial : seriale componente
//          manufacter : casa costruttrice
//          name : nome componente
export default function SolarPanelCard(props){
    return(
        <Card>
            <Card.Content>
            <Card.Header>{props.serial}</Card.Header>
            <Card.Meta>
                <span className='info'>Pannello Solare</span>
            </Card.Meta>
            <Card.Description>
                <Typography variant="body1">Produttore: {props.manufacter}</Typography>
                <Typography variant="body1">Nome: {props.name}</Typography>
                <Typography variant="body1">Potenza erogata: {props.power} W</Typography>
