        ...
        with db_session:
            try:
                ...
                _sensore = Sensore.select(lambda sensore: sensore.componente.seriale == request.json["seriale_sensore"])\
                                    .first()
                _acquisitore = Acquisitore.select(lambda acquisitore: acquisitore.componente.seriale == request.json["seriale_acquisitore"])\
                                    .first()
...
class GetCanali(Resource):
    ...
    def get(codice_stazione):
        ...
        if canali is not None:
            ...
            return jsonify(operationCode=404,message="Non ci sono canali")
